...
///   3. close = owner     automation PDA        → owner (rent + SOL)
///
/// Fee semantics: full refund for now. A protocol-fee deduction is
/// deferred to v5 once a treasury / admin-owned fee destination is
/// added to `Config`.
#[derive(Accounts)]
...
pub fn handler(ctx: Context<CloseAutomationSpl>) -> Result<()> {
    ...
    // lamports (e.g., link-fee-pool top-ups that were never drained).
